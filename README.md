...
> search space, the benchmark schema, the sequential Markov family and the calibration pilot
> are built and tested (124 tests). No optimiser exists yet, and no headline result is
> claimed. [`docs/design.md`](docs/design.md) is the protocol, including the hypotheses it is
> capable of falsifying and a log of every revision the design has been through.
...
function. Two published surrogates are implemented — **BOCS** (Baptista & Poloczek, ICML
2018: horseshoe prior, Gibbs sampling, Thompson sampling) and **FMQA** (Kitai et al., *Phys.
Rev. Research* 2020: a factorization machine minimised on an Ising machine).
...
`epochs` and family choice — and it means the multi-fidelity ladder needs rethinking before
Hyperband can be a fair baseline rather than a strawman.
...
- **`codecarbon` is reported as invalid on this machine.** green-rerank measured 0.0%
  utilisation under eight saturated cores and a hardcoded 10.000 W RAM figure. The cost unit
  is CPU-seconds; joules appear only as an explicitly-passed conversion, never as a
  measurement.
...
pytest tests/ --strict-companion        # 124 tests
ruff check .
